...
if (accordeons.length) {
  const expandItem = (target) => {
    ...
    if (isToggleMode) {
      ...
      prevItem && prevItem !== item ? prevItem.classList.remove("expanded") : null;
    }
  ...
  accordeons.forEach((accordeon) => {
    ...
    accordeon.classList.contains("--js--expanded-first-default") ? accordeon.querySelector(".accordeon__item:first-child").classList.add("expanded") : null;
...
if (sliders.length) {
  const breakpoints2 = (slider2) => {
    if (slider2.classList.contains("staff-preview-slider")) {
      ...
      return {
        680: {
          slidesPerView: slider2.classList.contains("--max-slides-2") ? 1 : slider2.classList.contains("--max-slides-3") ? 2 : 1,
          spaceBetween: slider2.classList.contains("--max-slides-3") ? 32 : 20
        },
        1024: {
          slidesPerView: slider2.classList.contains("--max-slides-2") ? 2 : slider2.classList.contains("--max-slides-3") ? 3 : 1,
          spaceBetween: slider2.classList.contains("--max-slides-2") ? 64 : slider2.classList.contains("--max-slides-3") ? 32 : 20
        }
      };
    }
  };
  sliders.forEach((slider2) => {
    ...
    new Swiper(slider2, {
      ...
      breakpoints: !slider2.classList.contains("--js--auto-fill") ? breakpoints2(slider2) : null,
      navigation: {
        nextEl: btnNext ? btnNext : null,
        prevEl: btnPrev ? btnPrev : null
      },
...
texts.forEach((text) => {
  ...
  clone.remove();
  const button = (_a = text.closest(".review-card__content")) == null ? void 0 : _a.querySelector(".review-card__button");
  if (button && !isOverflowing) {
    button.style.display = "none";
  }
});
ratings.forEach((rating) => {
  const value = parseFloat(rating.dataset.value) >= 0 ? parseFloat(rating.dataset.value) : MAX_VALUE;
